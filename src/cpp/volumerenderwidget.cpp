...
void VolumeRenderWidget::paintGL()
{
    ...
    program->setUniformValue("enterTexture", enterTexture);
    program->setUniformValue("exitTexture", exitTexture);
    glActiveTexture(GL_TEXTURE0 + enterTexture);
    glBindTexture(GL_TEXTURE_2D, enterTexture);
    glActiveTexture(GL_TEXTURE0 + exitTexture);
